...
namespace gazebo
{
  ...
  class RmotorPlugin : public ModelPlugin
  {
    ...
    public: virtual void Load(physics::ModelPtr _model, sdf::ElementPtr _sdf)
    {
      ...
      // Initialize ros, if it has not already bee initialized.
      	...
      	ros::init_options::NoSigintHandler);
			...
			ros::SubscribeOptions so = ros::SubscribeOptions::create<std_msgs::Float64>("/Right_Wheel_Motor/vel_cmd",1,boost::bind(&RmotorPlugin::OnRosMsg, this, _1),ros::VoidPtr(), &this->rosQueue);
			
			// Declare the node as a subscriber
			this->rosSub = this->rosNode->subscribe(so);

			this->rosQueueThread = std::thread(std::bind(&RmotorPlugin::QueueThread, this));
    }

    public: void SetVelocity(const double &_vel)
    {
      // Use no more than 0.45 Nm to achive the target vel
      this->model->GetJoint("Right_Wheel_Joint")->SetParam("fmax", 0, 0.45);
      ...
      this->model->GetJoint("Right_Wheel_Joint")->SetParam("vel", 0, rads);
    }
  ...
  GZ_REGISTER_MODEL_PLUGIN(RmotorPlugin)
}
#endif
