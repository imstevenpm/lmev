...
#include "lmev_gazebo/gazebo_ros_bumper.h"
...
void GazeboRosBumper::Load(sensors::SensorPtr _parent, sdf::ElementPtr _sdf)
{
  ...
  // MODIFICATION > Changing msg and topic to publish
  this->sensor_data = this->rosnode_->advertise<std_msgs::Int16>("Touch_Sensor", 1);
...
{
  if (this->sensor_data.getNumSubscribers() <= 0)
    return;
...
*/
  ...
  */
  ...
  // MODIFICATION > Determine if the button is pressed by evaluating the contact mgs created, and publishing result
  
  // Usually button is not pressed
  int button=0;
  
  // If any contact
  if (((this->contact_state_msg_.states).size())>0){
  
  	// In any state
  	for (int i=0; i<(this->contact_state_msg_.states).size(); i++){
  		
  		// In the y or x axis, depending on the vehicle orientation
  		// Is stronger than 0.5 N
  		// Then, the button is pressed
  		if (std::abs(this->contact_state_msg_.states[i].contact_normals[i].y)>0.5){
  			if (std::abs(this->contact_state_msg_.states[i].total_wrench.force.y)> 0.5){
  				button=1;
  				break;
  			}
  		}
  		else if ((std::abs(this->contact_state_msg_.states[i].contact_normals[i].x)>0.5)){
  			if (std::abs(this->contact_state_msg_.states[i].total_wrench.force.x)> 0.5){
  				button=1;
  				break;
  			}
  		}
  ...
  // Save button state data in msg
  msg.data=button;
  
  // Publish msg in topic
  this->sensor_data.publish(this->msg);
