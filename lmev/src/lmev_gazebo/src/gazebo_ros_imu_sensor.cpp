...
#include "lmev_gazebo/gazebo_ros_imu_sensor.h"
#include <iostream>
...
gazebo::GazeboRosImuSensor::GazeboRosImuSensor(): SensorPlugin()
{
  ...
  // MODIFICATION > Initializing values
  // Variable to consider accumulative angle and orientation
  Flag0=0;
  Flag1=0;
  Cont0=0;
  Cont1=0;
...
void gazebo::GazeboRosImuSensor::Load(gazebo::sensors::SensorPtr sensor_, sdf::ElementPtr sdf_)
{
  ...
  // MODIFICATION
  // Create topic to publish gyroscope data
  sensor_data= node->advertise<std_msgs::Int16>("Gyroscope_Sensor",1);
...
void gazebo::GazeboRosImuSensor::UpdateChild(const gazebo::common::UpdateInfo &/*_info*/)
{
  ...
  if(update_rate>0 && (current_time-last_time).Double() < 1.0/update_rate) //update rate check
    ...
    gyroscope_data = sensor->GetAngularVelocity();
		...
		// MODIFICATION > Commented this for performance issues
    ...
    // MODIFICATION > Finding euler angle
    
    // Variables to store euler angles
    ...
    tf::Matrix3x3 m(q);
		...
		// Extract only the z angle and convert it in degrees
		yaw=yaw*180/3.141593;
		
		// Algorithm to consider the rotation direction
		if ((Flag0>178) and (yaw<-178)){
			Cont0=Cont0+1;
			}
		else if ((Flag0<-178) and (yaw>178)){
			Cont0=Cont0-1;
			}
		Flag0=yaw;
		...
		if ((Flag1>719) and (yaw<361)){
			Cont1=Cont1+1;
			}
		else if ((Flag1<-719) and (yaw>-361)){
			Cont1=Cont1-1;
			}
  	...
  	// Mean: 0.0 Standard deviation: 1.5
  	...
  	gyroscope_angle = int(round(yaw+distribution(generator))+(360*Cont1));
    
    // Save gyroscope sensor data in msg
    msg.data= gyroscope_angle;
...
bool gazebo::GazeboRosImuSensor::LoadParameters()
{
  ...
  else
  {
	...
	// MODIFICATION > Reading noise custom values
  //NOISE
  if (sdf->HasElement("gaussian_noise_mean_g"))
  {
    gaussian_noise_mean =  sdf->Get<double>("gaussian_noise_mean_g");
  ...
  if (sdf->HasElement("gaussian_noise_std_g"))
  {
    gaussian_noise_std =  sdf->Get<double>("gaussian_noise_std_g");
    //ROS_INFO_STREAM("<gaussianNoise> set to: " << gaussian_noise);
  }
  else
  {
